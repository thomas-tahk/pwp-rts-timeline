...
- ssh into the server
    - For Mac and Linux: `ssh root@rts-timeline.info`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo ttahk`
    - replace ttahk with your own
- create password
    - `passwd ttahk`
- Move .ssh key file
    - `cp -r .ssh /home/ttahk`
- change ownership of the .ssh directory
    - `cd /home/ttahk`
    - `chown -R ttahk:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh ttahk@rts-timeline.info`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- create a directory for your deployment
    - `sudo mkdir /var/www/rts-timeline.info/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/rts-timeline.info/html`
...
- `sudo vim /etc/nginx/sites-available/rts-timeline.info`
...
server {
    ...
    root /var/www/rts-timeline.info/html;
    index index.html index.htm index.nginx-debian.html;

    server_name rts-timeline.info www.rts-timeline.info;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/rts-timeline.info /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/ttahk/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/ttahk/.ssh/github_deploy_key.pub >> /home/ttahk/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/ttahk/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H rts-timeline.info >> ~/.ssh/known_hosts 
         scp -r ./dist/* gkephart@rts-timeline.info:/var/www/rts-timeline.info/html
```
